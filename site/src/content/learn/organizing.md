...
Once a model outgrows a dozen objects, you stop finding things by orbiting around looking for them. The right-hand tray is the answer: the **Outliner** shows everything, **Entity Info** names and tags the selected thing, and **Tags** toggles whole categories on and off.

![A named group with a tag: Entity Info shows the name and tag chip, the Outliner shows the group row, and the Tags panel shows the tag tree with counts](/docs/organization.png)
...
## Entity Info: names and tags

Entity Info (Window ▸ Object Info, `⇧⌘O` / `Ctrl+Shift+O`) shows the single selected item:
...
With several things selected, Entity Info shows just the count; select one item to edit its details.
...
- **Add a tag** in Entity Info with the **+** button. Use `/` to nest: typing `Structure/Roof` creates (or reuses) a *Structure* parent with a *Roof* child.
...
- The watertightness badge in the status bar tells you *whether* something is leaky; the Outliner's dashed icons and Entity Info tell you *what*.
