...
3. The widget marking the plane reads mostly as an outline plus a small arrow along its normal — solid when the cut is active, dashed when it's turned off — with just enough fill to read as glass rather than empty space. It's always sized to frame the whole visible model, from any angle, and grows automatically as you add geometry beyond its current edges.
