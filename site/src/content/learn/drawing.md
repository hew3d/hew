...
**Deleting a line or curve.** With the Select tool, click any sketch line to select it — a facet of a drawn arc or circle selects the whole curve — and press `Delete`. If the line separated two regions, they merge; if it closed a region, the region opens back up. Deleting a shape's last line removes it entirely. Lines bordering a region you've already pulled into a solid delete like any other — the solid remembers the area it stands on, so nothing under it ever becomes extrudable, however the surrounding regions split or merge (delete the circle at the bottom of a hole and the hole's floor joins the solid's footprint). One consequence to know: deleting a line between a solid's footprint and open ground merges the two into one region that can't be extruded while the solid stands — redraw the line to get the open part back, or delete the solid. Delete the solid itself and its footprint is released: the outline returns to the sketch, fully editable again.
