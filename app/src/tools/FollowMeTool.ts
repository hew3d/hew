 ...
 *      consumed exactly like an extrusion's; the path stays.
...
type Stage =
  | { kind: 'pick-path' }
  | { kind: 'pick-profile'; path: PathTarget }
...
export class FollowMeTool implements Tool {
  ...
  constructor(
    ...
    if (preselected !== null) {
      this.stage = { kind: 'pick-profile', path: preselected }
  ...
  statusHint(): string {
    return this.stage.kind === 'pick-path'
      ? 'Click the path to follow — a drawn line or curve, or a solid face to run around.'
      : 'Click the profile to sweep along the path — it must sit across the path’s end.'
  }
  ...
  onPointerDown(_snap: Snap | null, ray: Ray): void {
    if (this.stage.kind === 'pick-path') {
      ...
      if (path === null) return
      this.stage = { kind: 'pick-profile', path }
      this._highlightPath(path)
      return
    }

    // pick-profile: a region click commits; anything else is a no-op so a
    // stray click can't silently discard the picked path.
