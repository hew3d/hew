 ...
 * tabletop's TOP face for BOTH profiles (pinned by the kernel spec
 * `follow_me_molding_around_the_maintainer_tabletop`) — but WHICH face becomes
 * the path: only the flat top (or bottom) face carries an upright profile, and
 * a pick ray through the standing profile lands on a side face the kernel
 * correctly refuses.
 *
 * This drives the SOLID-FACE commit through the real Open path + wasm binding,
 * grounded in the actual file: load it, then sweep BOTH profiles around the
 * tabletop's top face and assert a watertight molding commits with the
...
test('Follow Me: both profiles sweep a watertight molding around the tabletop top face (maintainer file)', async ({
  page,
}) => {
  await boot(page)
  ...
  // Exactly the two upright profiles commit a watertight molding; the flat
  // ground rectangle (parallel to the top face) is refused.
